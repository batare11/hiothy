...
DOUBAO_MODEL=方舟控制台创建的推理接入点ID
DOUBAO_TIMEOUT=60
```

`DOUBAO_MODEL` 通常应填写推理接入点 ID，而不是
`doubao-1.5-vision-pro-32k` 展示名称。
